...
def run_pipeline(csv_path: str, city_name: str, fast: bool, force: bool, no_train: bool):
    if not no_train:
        ...
        knn_missing  = not exists["knn"]
        lstm_missing = not exists["lstm"]

        if force:
            do_knn, do_lstm = True, True
        elif knn_missing or lstm_missing:
            do_knn  = True if knn_missing  else None
            do_lstm = True if lstm_missing else None

            if knn_missing:
                print(f"\n  [KNN]  Model za {city_name} ne postoji - treniram automatski.")
            if lstm_missing:
                print(f"\n  [LSTM] Model za {city_name} ne postoji - treniram automatski.")

            ask_knn, ask_lstm = _ask_train(city_name, fast)
            if do_knn  is None: do_knn  = ask_knn
            if do_lstm is None: do_lstm = ask_lstm
        else:
            do_knn, do_lstm = _ask_train(city_name, fast)

        if do_knn or do_lstm:
            train_city(csv_path, city_name, train_knn=do_knn, train_lstm=do_lstm, fast=fast)
    ...
    run_next_day_prediction(csv_path, city_name, knn_m, knn_s, lstm_m, lstm_s, output_dir=RESULTS_DIR)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fast",     action="store_true", help="Brzi KNN rezim (bez GridSearchCV)")
